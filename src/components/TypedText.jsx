...
export default function TypedText({ className = "" }) {
  ...
  useEffect(() => {
    ...
    if (!isDeleting && displayed.length < fullText.length) {
      ...
      timeout = setTimeout(() => setIsDeleting(true), DELAY_AFTER_TYPED);
  ...
  const nextItems = Array.from({ length: texts.length - 1 }, (_, i) =>
    texts[(textIndex + i + 1) % texts.length]
  );
        ...
        >
          {nextItems.map((t, i) => (
            <div
              ...
              style={{
                padding: "7px 0",
                borderBottom: i < nextItems.length - 1 ? "1px solid #2a375a" : "none",
                letterSpacing: "0.2px",
                fontWeight: 500,
                fontFamily: "inherit",
                borderRadius: "8px",
